...
class ANN:
    ...
    def _backward(self, cache, y_true):
        ...
        for l in reversed(range(self.L)):
            ...
            if l > 0:
                dA = dZ @ self.W[l].T
                if dA.ndim == 1:
                    dA = dA.reshape(-1, self.sizes[l])

        return grads_W, grads_b

    def _update(self, grads_W, grads_b, reg_grads):
        for l in range(self.L):
            if isinstance(reg_grads, list) and len(reg_grads) > l:
                gW = grads_W[l] + reg_grads[l]
            else:
                gW = grads_W[l]
            self.W[l] -= self.lr * gW
    ...
    def fit(self, X, y):
        ...
        y = np.asarray(y, dtype=float).reshape(-1, self.sizes[-1])
        ...
        for epoch in range(self.epochs):
            ...
            for step in range(steps_per_epoch):
                s, e = step*self.bs, min(N, (step+1)*self.bs)
                Xb, yb = Xs[s:e], ys[s:e].reshape(-1, self.sizes[-1])
                ...
                batch_loss = data_loss + reg_loss / max(1, steps_per_epoch)
